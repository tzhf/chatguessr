...
export default class PostProcessingController {
  ...
  assemblePasses() {
    ...
    const knownUniforms: Map<string, Uniform> = new Map()
    for (const effectName of shaderToggles) {
  ...
  fixupStateBeforeSet(newState: PostProcessingState) {
    ...
    for (const toggle of mutuallyExclusiveShaderToggles) {
  ...
  passShaderInfoAndUniforms() {
    const shaderInfo = this.assemblePasses() as ShaderInfo
    if (this.handler) {
