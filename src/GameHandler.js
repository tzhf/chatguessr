        ...
        try {
            ...
            if (!this.#game.isMultiGuess) {
                ...
                if (settings.showHasGuessed) {
                    await this.#backend.sendMessage(
                        `${flags.getEmoji(guess.flag)} ${userstate["display-name"]} has guessed`
                    );
                }
            } else {
                ...
                if (!guess.modified) {
                    ...
                    if (settings.showGuessChanged) {
                        await this.#backend.sendMessage(
                            `${flags.getEmoji(guess.flag)} ${userstate["display-name"]} guess changed`
                        );
                    }
                }
            }
            ...
            if (err.code === "alreadyGuessed") {
                ...
                if (settings.showSubmittedPreviousGuess) {
                    await this.#backend.sendMessage(`${userstate["display-name"]} you submitted your previous guess`);
                }
            } else {
                console.error(err);
            }
    ...
    async #handleMessage(userstate, message) {
        ...
        if (message === settings.clearUserStatsCmd) {
            ...
            if (dbUser) {
                ...
                await this.#backend.sendMessage(
                    `${flags.getEmoji(dbUser.flag)} ${userstate["display-name"]} 🗑️ stats cleared !`
                );
            } else {
                await this.#backend.sendMessage(`${userstate["display-name"]} you've never guessed yet.`);
            }
